...
using LinearAlgebra
    ...
    function plot_ellipse_from_covariance_matrix(cov_matrix; center=(0, 0),n_points=100, factor, kwargs...)
    ...
    x_values = [center[1] + factor[1] * major_length * cos(θ) * major_axis[1] + factor[1] * minor_length * sin(θ) * minor_axis[1] for θ in θ_values]
    y_values = [center[2] + factor[1] * major_length * cos(θ) * major_axis[2] + factor[1] * minor_length * sin(θ) * minor_axis[2] for θ in θ_values]
